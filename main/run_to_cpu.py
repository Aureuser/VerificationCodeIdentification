...
def main():
    ...
    train_img_loader = DataLoader(train_imageset, batch_size=128, shuffle=True)
    test_imageset = ImageDataset(root='../data/Discuz/', imgs=test_img, transform=transform)
    test_img_loader = DataLoader(test_imageset, batch_size=128, shuffle=False)
    model = CNN_model()
    train(model=model, train_loader=train_img_loader, test_loader=test_img_loader,
          epochs=1024, lr=0.05, use_cuda=False)
